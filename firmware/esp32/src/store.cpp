...
// LittleFS is not thread-safe: append runs in SensorTask, replay in MqttTask.
...
#if OO_STORE_ENABLE
  if (!gReady || !gMx) return false;
  if (xSemaphoreTake(gMx, pdMS_TO_TICKS(200)) != pdTRUE) return false;
  bool ok = false;
  File f = LittleFS.open(PATH, "a");
  if (f) {
    if (f.size() <= OO_STORE_MAX) {                 // cap: stop appending (oldest survives)
...
#if OO_STORE_ENABLE
  if (!gReady || !gMx) return 0;
  if (xSemaphoreTake(gMx, pdMS_TO_TICKS(100)) != pdTRUE) return 0;
...
#if OO_STORE_ENABLE
  if (!gReady || !gMx) return 0;
  if (xSemaphoreTake(gMx, pdMS_TO_TICKS(500)) != pdTRUE) return 0;
  size_t n = 0;
  File f = LittleFS.open(PATH, "r");
  if (f) {
    File tmp = LittleFS.open(TMP, "w");             // collects the UNSENT remainder
    if (!tmp) { f.close(); xSemaphoreGive(gMx); return 0; }   // can't scratch -> keep buffer intact
