...
static void buildConsolidatedReading(const OoProfile* prof, uint64_t epoch) {
  JsonDocument d;
  d["nodeId"] = ooId().device;
  d["ts"] = epoch;
  JsonObject values = d["values"].to<JsonObject>();
  for (size_t i = 0; i < prof->count; i++) {
    const OoChannel& ch = prof->channels[i];
    OoReading r = ooReadChannel(ch);
    if (!isnan(r.value)) {
      values[ch.channel] = r.value;
      if (isEternity && strcmp(ch.channel, "dga_h2_ppm") == 0) ooTrendPush(DGA_KEY, epoch, r.value);
      OoAlarmOut a = ooAlarmEval(i, ch, r.value, epoch);
      if (a.fire) buildEdgeAlarm(ch, r.value, a.sev);
    }
  }
  char buf[384]; size_t n = serializeJson(d, buf);
  // Publishes to P = {OO_TOPIC_ROOT}/{tenant}/{product}/{device} or directly if PFX
  publishOrStore("", buf, n, /*qos*/1, /*prio*/0);
}
#endif
...
static void sensorTask(void*) {
  ...
  for (;;) {
    ...
    if (now - lastSample >= period) {
      lastSample = now;
#if OO_NODERED_COMPAT
      buildConsolidatedReading(profile, epoch);
#else
      float impactG = 0;
