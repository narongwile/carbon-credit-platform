...
console.log('####### Scenario B: already-authenticated session, DIRECT navigation to /admin (no login form submit), stale selectedOrgId=org-1 persisted #######');
{
  ...
  await page.goto('http://localhost:3902/', { waitUntil: 'domcontentloaded' });
  await page.evaluate(() => {
  ...
  await page.goto('http://localhost:3902/admin/transformers/detail/?id=tr-222', { waitUntil: 'networkidle' });
