...
export default function AnalyticsChart({ data, nodeName }: Props) {
  ...
  return (
    <div className="bg-[#1e293b] rounded-xl border border-slate-700 p-6 shadow-lg flex-1 flex flex-col relative overflow-hidden">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 z-10 gap-3 sm:gap-0">
        <h3 className="text-lg font-bold text-white tracking-wide">Performance Analytics</h3>
        <span className="text-[10px] sm:text-xs font-bold text-slate-400 bg-slate-800 px-3 py-1 rounded-full uppercase tracking-widest border border-slate-700">Viewing {nodeName}</span>
      </div>

      <div className="flex-1 grid grid-rows-[2fr_1fr] gap-6 z-10 w-full min-h-[500px] sm:min-h-[400px]">
        ...
        <div className="relative border-b border-slate-700/50 pb-4">
          ...
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={renderData} margin={{ top: 20, right: 10, left: -20, bottom: 0 }}>
              ...
              <XAxis dataKey="date" stroke="#64748b" fontSize={10} tickFormatter={(val) => val.substring(5, 10)} minTickGap={40} tickMargin={12} height={30} />
