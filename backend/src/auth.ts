...
// Attach claims from a Bearer token (no rejection here — guards decide). Falls
// back to a ?token= query param for contexts that can't set headers, e.g. an
// <img src> loading a protected floor-plan image.
export function authMiddleware(req: Request, _res: Response, next: NextFunction) {
  const h = req.headers.authorization || ''
  const tok = h.startsWith('Bearer ') ? h.slice(7) : (typeof req.query.token === 'string' ? req.query.token : '')
