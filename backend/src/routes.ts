...
import {
  ...
  getUser, getPrefs, putPrefs, updateMe,
...
router.put('/me/config', async (req, res) => {
  const uid = req.auth?.userId || req.header('x-user-id')
  if (!uid) return res.status(401).json({ error: 'authentication required' })
  const { name, phone, prefs } = req.body ?? {}
  if (name !== undefined || phone !== undefined) {
    await updateMe(uid, name || '', phone || '')
  }
  if (prefs !== undefined) {
    const existing = await getPrefs(uid)
    await putPrefs(uid, { ...existing, ...prefs })
  } else if (!name && !phone && Object.keys(req.body ?? {}).length > 0) {
    // Fallback: if they just sent a raw prefs object without name/phone (like previous behavior)
    const existing = await getPrefs(uid)
    await putPrefs(uid, { ...existing, ...req.body })
  }
  res.json({ ok: true })
})
