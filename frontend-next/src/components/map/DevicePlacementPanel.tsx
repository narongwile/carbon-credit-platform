...
export default function DevicePlacementPanel({
  ...
  if (!open) {
    return (
      <button onClick={() => onOpenChange(true)} disabled={!!session}
        className="absolute bottom-4 left-4 z-[1000] flex items-center gap-2 px-3.5 py-2.5 rounded-xl text-xs font-semibold text-white shadow-2xl disabled:opacity-50 transition-all hover:scale-105 active:scale-95"
        style={gradient} title="Set device positions on the map">
        <MapPin size={14} /> Set device positions
        {unplaced.length > 0 && (
          <span className="text-[10px] px-2 py-0.5 rounded-full font-bold" style={{ background: 'rgba(0,0,0,0.35)' }}>{unplaced.length} unlocated</span>
        )}
      </button>
    )
  }
  ...
  return (
    <div className="absolute bottom-4 left-4 z-[1000] w-[320px] max-h-[calc(100%-100px)] rounded-xl shadow-2xl flex flex-col" style={surface}>
