...
export default function NameplateEditor({
  nodeId, current, onClose, onSaved,
}: {
  nodeId: string
  ...
  const save = async () => {
    if (!canSave) return
    setSaving(true)
    // Blank = clear that field; every field is independently optional.
        ...
        <div className="p-5 space-y-3 max-h-[65vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-2 gap-3">
            <label className="block col-span-2">
              ...
              <input value={form.manufacturer} onChange={set('manufacturer')} placeholder="e.g. ABB" maxLength={120}
            ...
            <label className="block">
              ...
              <input value={form.ratedKva} onChange={set('ratedKva')} inputMode="decimal" placeholder="e.g. 2500"
            ...
            <label className="block">
              ...
              <input value={form.voltageClass} onChange={set('voltageClass')} placeholder="e.g. 22kV/0.4kV" maxLength={64}
                className="w-full rounded-lg px-3 py-2 text-sm text-white outline-none" style={inset} />
            </label>
            <label className="block">
              <span className="block text-[10px] text-slate-500 mb-1 uppercase tracking-wider">Cooling type</span>
              <input value={form.coolingType} onChange={set('coolingType')} list="nameplate-cooling-types" placeholder="e.g. ONAN" maxLength={32}
                className="w-full rounded-lg px-3 py-2 text-sm text-white outline-none" style={inset} />
