...
import { Building2, Zap, Thermometer, Droplet, MapPin, Leaf, AlertTriangle, Activity, HeartPulse, Car, ArrowUpRight, TrendingUp, FileBarChart } from 'lucide-react'
        ...
        return (
          <div key={site.id} className="rounded-2xl p-5 space-y-4" style={surface}>
            ...
            <div className="flex items-start justify-between">
              ...
              <div className="flex items-center gap-2">
                <Link
                  href={`/admin/reports?siteId=${encodeURIComponent(site.id)}`}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold text-amber-300 bg-amber-500/10 hover:bg-amber-500/20 border border-amber-500/25 transition-all shadow-sm"
                  title={`Generate site compliance and telemetry report for ${site.name}`}
                >
                  <FileBarChart size={13} className="text-amber-400" />
                  <span>Report →</span>
                </Link>
                <Link
