...
export default function SettingsPage() {
  ...
  const onLogo = (file?: File) => {
    ...
    reader.onload = async () => {
      ...
      if (apiEnabled) {
        const ok = await api.postOrg({ id: selectedOrgId, name: orgName, logoUrl: dataUrl })
        if (!ok) { toast.error('Failed to save logo'); return }
      }
  ...
  const removeLogo = async () => {
    if (apiEnabled) {
      const ok = await api.postOrg({ id: selectedOrgId, name: orgName, logoUrl: '' })
      if (!ok) { toast.error('Failed to remove logo'); return }
    }
