...
function connect() {
  ...
  // Authenticate the socket: the backend registers this connection's org from the
  // JWT and only then streams telemetry to it (per-org fan-out, no cross-org leak).
  ws.onopen = () => {
    try {
      const tok = typeof localStorage !== 'undefined' ? localStorage.getItem('oneops_token') : null
