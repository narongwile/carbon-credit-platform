...
\begin{tikzpicture}[node distance=8mm and 10mm]
  ...
  \node[fproc, below=of b3] (b4) {Store NVS $\to$ reboot};
  % main spine (down)
  ...
  \draw[a,crossEdge] (safe.south) |- (m3.west) node[pos=0.7,above,font=\tiny]{report-only};
  ...
  \draw[a] (b3)--(b4);
  \draw[a,gray] (b4.south) |- ($(s1.west)+(0,-0.1)$) node[pos=0.85,below,font=\tiny]{reboot};
...
\end{tikzpicture}}
\caption{Boot \& provisioning with Safe-Mode diversion, bootstrap retry, and link/MQTT backoff loops.}
\end{figure}
...
\begin{tikzpicture}
  ...
  \node[tk={halCol}{halEdge}, text width=2.0cm] (q1) at (3.1,0) {sample queue};
  \node[tk={crossCol}{crossEdge}] (al) at (6.2,0.9) {\textbf{EdgeAlarmTask}\\threshold breach$\to$enqueue};
  \node[tk={appCol}{appEdge}] (te) at (6.2,-0.9) {\textbf{TelemetryTask}\\envelope (+quality)};
  \node[tk={sysCol}{sysEdge}, text width=3.0cm] (buf) at (3.1,-2.0)
       {Ring buffer / SD\\FIFO drop-OLDEST (tele)\\NEVER drop alarms};
  \node[tk={appCol}{appEdge}, text width=3.1cm] (eg) at (9.4,0)
       {\textbf{EGRESS QUEUE}\\priority Alarm>Telemetry\\single consumer};
  \node[tk={drvCol}{drvEdge}] (link) at (12.5,0) {\textbf{LinkMgr}\\failover + reconnect};
  \node[tk={cloudCol}{cloudEdge}, text width=2.0cm] (br) at (15.2,0) {EMQX\\(mTLS)};
  % downlink row
  \node[tk={profCol}{profEdge}] (cfg) at (1.5,-3.8) {\textbf{ConfigTask}\\thresholds, rates};
  \node[tk={profCol}{profEdge}] (cmd) at (4.7,-3.8) {\textbf{CmdTask}\\reboot/calibrate};
  \node[tk={sysCol}{sysEdge}] (ota) at (7.9,-3.8) {\textbf{OTATask}\\fetch$\to$verify$\to$A/B};
  \node[tk={crossCol}{crossEdge}, text width=3.2cm] (wdt) at (11.4,-3.8)
       {\textbf{Watchdog (TWDT)}\\per-task heartbeat flag\\miss $\to$ targeted reset};
  \draw[flow] (sens)--(q1); \draw[flow,crossEdge] (q1)--(al); \draw[flow,appEdge] (q1)--(te);
  \draw[flow,crossEdge] (al)--(eg) node[elab,midway]{high}; \draw[flow,appEdge] (te)--(eg) node[elab,midway]{norm};
  \draw[flow] (eg)--(link) node[elab,midway]{drain}; \draw[flow] (link)--(br);
  \draw[flowd,sysEdge] (te)--(buf) node[elab,midway]{offline}; \draw[flowd,sysEdge] (buf) to[bend right=10] (eg);
  \draw[flowd,cloudEdge] (br) to[bend right=22] (link);
  \draw[flow] (link.south) |- (cfg.east); \draw[flow] (link.south) |- (ota.east);
  \draw[flowd,profEdge] (cfg) to[bend left=12] (al) node[elab,pos=0.5]{thr};
  \draw[flowd,profEdge] (cfg) to[bend left=8] (sens) node[elab,pos=0.5]{rate};
  \foreach \t in {sens,al,te,link,ota} \draw[flowd,crossEdge] (\t.south) -- ++(0,-0.25) -- (wdt.north);
\end{tikzpicture}}
...
\begin{tikzpicture}
  \node[circle, fill=black, inner sep=2.2pt] (i) at (-5.6,2.2){};
  \node[stnode={hwCol}{hwEdge}] (cn) at (-3.2,2.2){\textbf{CONNECTING}\\\tiny Wi-Fi$\to$4G$\to$LoRa};
  \node[stnode={drvCol}{drvEdge}, text width=3.4cm] (co) at (1.2,2.2)
       {\textbf{CONNECTED}\\\tiny birth $\cdot$ drain egress $\cdot$ flush log $\cdot$ heartbeat};
  \node[stnode={sysCol}{sysEdge}] (rc) at (5.6,2.2){\textbf{RECONNECTING}\\\tiny same transport};
  \node[stnode={profCol}{profEdge}, text width=3.6cm] (bk) at (1.2,-0.6)
       {\textbf{BACKOFF}\\\tiny exp backoff (cap+jitter)};
  \node[stnode={crossCol}{crossEdge}, text width=3.6cm] (of) at (-3.6,-0.6)
       {\textbf{OFFLINE}\\\tiny store-and-forward $\cdot$ buffer logs $\cdot$ wake};
  \node[stnode={cloudCol}{cloudEdge}, text width=3.4cm] (lg) at (5.8,-0.6)
  ...
  \draw[t,crossEdge] (cn) to[bend right=14] (bk) node[midway,left,font=\tiny]{all failed};
  \draw[t,profEdge] (co) to[bend left=8] (rc) node[midway,above,font=\tiny]{keepalive miss};
  \draw[t,drvEdge] (rc) to[bend left=8] (co) node[midway,below,font=\tiny]{ok};
  \draw[t,crossEdge] (rc) to[bend left=14] (bk) node[midway,right,font=\tiny]{retry>N};
  \draw[t,sysEdge] (bk) to[bend right=10] (cn) node[midway,left,font=\tiny]{timer$\to$next};
  \draw[t,crossEdge] (bk)--(of) node[midway,above,font=\tiny]{max backoff};
  \draw[t,sysEdge] (of) to[bend left=18] (cn) node[midway,left,font=\tiny]{wake/retry};
  \foreach \s in {cn,rc,bk,of} \draw[t,cloudEdge,densely dashed] (\s) -- (lg);
  \draw[t,drvEdge,densely dashed] (co)--(lg) node[midway,above,font=\tiny]{flush};
\end{tikzpicture}}
